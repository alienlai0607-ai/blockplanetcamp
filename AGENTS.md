...
| `junkbot-tournament/` | 廢材機器人大賽系統（東橋／北區分流、選廢秀、1 分鐘淘汰賽） |
...
| `junkbot-state&campus=dongqiao|north` | 公開 | 廢材機器人大賽指定校區的隊伍、影片、賽程與即時戰況 |
...
| `{action:'junkbot-state-set', password:'block', campus, state}` | 依校區儲存廢材機器人賽事狀態（≤48000 字元） |
| `{action:'junkbot-video-upload', password:'block', campus, teamId, teamName, filename, mimeType, dataUrl}` | 選廢秀影片存入 Drive（前端限制 18MB） |
...
- 新建立的 6 隊賽事採「三輪爬升積分＋前兩名總決賽」：每隊固定 3 場；每輪結束後依最新排名自動配對下一輪，優先安排相近戰績且盡量不重複對手；排名依勝場 → 對手強度 → 得失分差 → 總得分；三輪後前兩名進總決賽
- 6 人／2 隊仍直接進行冠軍戰；3 隊等無法成雙的隊數使用單組循環。已完成的舊橘藍雙組賽事會由既有 `poolId` 自動辨識為 `two-pools` 並保留歷史冠軍，不會被新制改寫
- 同一網址先選身份：觀眾模式免密碼，只顯示今日即時倒數、比分、隊員、積分榜、下一場與冠軍畫面，且約每 6.5–8.3 秒自動更新；比賽單位控制需輸入密碼 `block`，才顯示駕照、報到、分組、計分、時間與音樂控制
- 2026-07-24 起觀眾 GET（`drone-state`／`drone-pilots`）走 Apps Script 純讀取快車道，不搶全域鎖、也不掃優惠券；前端 8 秒逾時、自動重試、使用最近一次快取先開畫面，輪詢加入 6.5–8.3 秒隨機間隔，避免多人同時開啟時排隊卡住
- 正式賽開啟後，控制台會把 `live`（matchId/status/secondsLeft/updatedAt/scores）隨賽事狀態寫入 `無人機賽事`；觀眾端依時間戳推算秒數。練習賽不會同步到觀眾端，也不會改正式賽程
...
- 音效：比賽畫面可試聽並選擇 4 首一般賽事音樂與 3 首最後 30 秒音樂（全部 CC0）；預設「Boss Fight Bounce／星球彈跳」搭配「Final Stand Max／最終決戰 MAX」。開場 READY → 3 → 2 → 1 只播放專屬充能與逐秒重擊音效，背景音樂保持靜音，時間進入 3:00 才從第一拍播放；倒數到 0:30 時一般音樂立即停止、終局音樂從第一拍重新播放，並疊加換曲警報、Web Audio 重鼓、心跳與急促琶音；最後 10 秒進入第三階段，數字放到滿版、畫面強烈震動並逐秒重擊；來源與授權記錄在 `drone-soccer/AUDIO-CREDITS.md`
- 終局判定：時間歸零後依隊伍比分自動顯示勝隊、三位隊員與滿版彩帶，再由裁判確認勝隊晉級／奪冠；若最高分同分則顯示決勝加分，不會隨機選勝隊，裁判加上致勝分後立即重新判定
- 賽場視覺：一般比賽／暫停畫面使用明亮的薄荷綠、天空藍與暖黃色「白天星球競技場」，深綠字與半透明白色隊伍卡片；只有最後 30 秒切換為深紅色緊張戰鬥模式
...
## 🤖 廢材機器人大賽（junkbot-tournament/，2026-07-24 建立）

- 營隊官網老師系統選單中的入口緊接在「無人機足球晉級賽」下方。
- 同一網址先選身份與校區：觀眾免密碼、只能觀看；比賽單位需密碼 `block`，再選東橋或北區。
- 東橋 `dongqiao` 與北區 `north` 使用獨立賽事資料列；隊伍、選手、選廢秀影片、即時倒數與晉級結果不可跨校區混用。
- 名單格式是一隊一位選手，可逐隊新增，也可用 `隊名｜選手名字`、tab 或逗號批次貼上。正式完整賽程為 16 隊；不足 16 隊時系統自動處理輪空。
- 賽制：16 強 → 8 強 → 4 強 → 冠亞軍賽，另依簡章獎項加入四強落敗隊伍的季軍賽。
- 主賽程以「晉級爬升圖」呈現：16 強位於左下，依 LEVEL 1–4 向右上爬升至冠軍最高點；季軍賽獨立顯示為四強落敗支線。
- 每場依簡章固定 1 分鐘、不停錶；READY → 3 → 2 → 1 使用 Web Audio 儀式音效，最後 10 秒加強數字、震動與逐秒聲響。
- 勝負不採比分自動判斷。評審可在比賽中或時間到後選擇勝隊與原因，整隊自動推進下一輪；每場最多重賽一次。
- 判定選項涵蓋：掉出場外、停止移動超過 10 秒、10 秒內未離開起始區、時間到後較接近對方基地、惡意觸碰／違規、棄權與其他現場判定。
- 選廢秀：比賽單位可貼 YouTube／Google Drive／mp4 網址，或上傳 18MB 以下影片；上傳檔存至 Drive `2026廢材選廢秀影片/{校區}`，觀眾點隊名即開啟影片。
- `junkbot-state` 同樣走公開純讀取快車道；觀眾前端先顯示空白／最近戰況，再以 8 秒逾時與 6.5–8.3 秒間隔背景重試，不讓 Apps Script 冷啟動卡住整頁。
- Sheet 分頁：`廢材機器人賽事`，欄位為校區 Key／校區名稱／賽事 Payload／更新時間；每校區一列。
- `?demo=1` 使用瀏覽器 localStorage 測試賽程，不寫入正式 Sheet；示範模式不支援上傳影片，但可貼影片網址。
